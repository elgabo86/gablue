...
- **Composants de chemin invalides Windows (point/espace final)** : Windows interdit les noms de dossiers finissant par un point ou un espace, règle émulée par Wine lors de la conversion des chemins → le fichier devient introuvable (ex: ISO montée avec un label finissant par un point : `/run/media/user/GAME.BUILD.123./setup.exe`). `run_classic_mode` (gwine) normalise le chemin avec `realpath -s` (normalise `.`/`..` sans résoudre les symlinks, pour ne pas casser un contournement manuel par symlink) puis détecte les composants invalides dans le dirname (patterns `*"./"* ` et `*" /"*` sur le chemin suffixé d'un `/`). Si détecté, le lancement est redirigé via un symlink temporaire (`$gwine_temp_base/gamedir` → dossier réel de l'exe) : Wine ne résout pas les symlinks, le chemin vu ne contient plus de composant invalide. Limite connue : le CWD physique (`getcwd()`) garde le nom réel → les accès relatifs au CWD côté Windows restent dégradés (bénin pour les installateurs). `gwine_temp_base` est volontairement **globale** (pas `local`) pour rester visible dans les traps EXIT/INT/TERM qui la nettoient (l'ancien trap EXIT dédié au temp non-ASCII était écrasé par les traps suivants → fuite corrigée au passage, les deux mécanismes partagent désormais `gwine_temp_base`)
