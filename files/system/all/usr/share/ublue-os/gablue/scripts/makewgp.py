...
class CreateWGPThread(QThread):
    ...
    def create_config_files(self):
        ...
        if self.config.get('pds'):
            with open(os.path.join(self.game_dir, '.pds'), 'w') as f:
                f.write(self.config['pds'])
        elif os.path.exists(os.path.join(self.game_dir, '.pds')):
            os.remove(os.path.join(self.game_dir, '.pds'))
            ...
            with open(os.path.join(self.game_dir, '.extrapath'), 'w') as f:
                for item_type, rel_path in self.config['extras']:
                    ...
                    if os.path.exists(source):
                        ...
                        # Créer le symlink vers /tmp/wgp-extra
                        # Si le source est déjà un symlink vers l'extérieur, on le garde tel quel
                        if os.path.islink(source):
                            link_target = os.readlink(source)
                            if os.path.isabs(link_target) and not link_target.startswith(self.game_dir):
...
class WGPWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # ProgramData (.pds) - après Arguments
        pds_group = QGroupBox("ProgramData (.pds)")
        pds_group.setToolTip("Nom du dossier ProgramData à mapper (ex: MonJeu)")
        pds_layout = QVBoxLayout(pds_group)
        pds_layout.setContentsMargins(8, 12, 8, 8)

        self.pds_input = QLineEdit()
        self.pds_input.setPlaceholderText("Nom du dossier ProgramData...")
        pds_layout.addWidget(self.pds_input)

        left_layout.addWidget(pds_group)
        
        # Options du paquet (Fix manette, Compression et Full Overlay)
        ...
        temps_group.setToolTip("Stockées dans /tmp/wgp-temp (effacés à la fermeture)")
    ...
    def load_game_directory(self, game_dir):
        ...
        # Charger le chemin .pds depuis .pds si existe
        pds_file = os.path.join(self.game_dir, '.pds')
        if os.path.exists(pds_file):
            with open(pds_file, 'r') as f:
                pds_path = f.read().strip()
                self.pds_input.setText(pds_path)
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for f in os.listdir(temp_dir):
                    if f.startswith(base_name) and f.endswith('.png'):
                        ...
                        self.temp_icons.append(full_path)  # Marquer pour nettoyage
        ...
        for exe_path in exe_files[:10]:
            try:
                ...
                if result.returncode == 0 and os.path.exists(png_file):
                    ...
                    self.temp_icons.append(png_file)  # Marquer pour nettoyage
    ...
    def cleanup_temp_icons(self):
        ...
        # Nettoyer aussi les anciens fichiers wgp_icon_ oubliés dans /tmp
        temp_dir = tempfile.gettempdir()
        try:
            for f in os.listdir(temp_dir):
                if f.startswith('wgp_icon_') and (f.endswith('.png') or f.endswith('.ico')):
                    try:
    ...
    def add_item(self, item_type, item_subtype):
        ...
        # Mettre à jour l'affichage (ne pas persister - uniquement à la création du WGP)
        if item_type == 'save':
            self.update_saves_list()
        elif item_type == 'extra':
            self.update_extras_list()
    ...
    def remove_item(self, item_type):
        """Supprime un élément de la liste (ne pas persister - uniquement à la création du WGP)"""
        if item_type == 'save':
    ...
    def update_temps_list(self):
        """Met à jour la liste des fichiers temporaires"""
    ...
    def toggle_full_temp(self, state):
        ...
        # Ne pas persister - uniquement à la création du WGP
    ...
    def start_create_wgp(self):
        ...
        config = {
            ...
            'extras': self.extras,
            'temps': self.temps,
            'pds': pds_path if pds_path else None
        }
    ...
    def on_finished(self, success, message):
        ...
        if success:
            ...
            self.cleanup_temp_icons()  # Nettoyer avant de fermer
            self.close()
        else:
            self.cleanup_temp_icons()  # Nettoyer aussi en cas d'échec
