...
# -*- coding: utf-8 -*-
"""
LGP Creator - Création de paquets Linux Game Packs
Interface graphique PySide6 (Qt) pour la création de paquets LGP

Usage: makelgp.py <dossier_du_jeu>
"""
...
class CreateLGPThread(QThread):
    """Thread pour créer le LGP sans bloquer l'interface"""
    ...
    def run(self):
        try:
            ...
            if result.returncode == 0:
                ...
                self.cleanup()
                self.finished.emit(True, lgp_file)
            else:
    ...
    def _process_saves_and_extras(self):
        ...
        # Sauvegarder les infos des symlinks dans un fichier pour pouvoir les restaurer
        if symlinks_backup:
            symlinks_file = os.path.join(self.game_dir, '.symlinks_backup')
            with open(symlinks_file, 'w') as f:
                for rel_path, target in symlinks_backup.items():
                    f.write(f"{rel_path}|{target}\n")
            print(f"DEBUG: Saved {len(symlinks_backup)} symlinks to {symlinks_file}")
    ...
    def create_squashfs(self, lgp_file):
        ...
        # Fonction pour surveiller la taille du fichier LGP en parallèle
        import threading
        import time
        ...
        def watch_lgp_size():
            """Surveille la taille du fichier LGP et met à jour progress_data"""
            last_size = 0
            while not progress_data['finished']:
                try:
                    if os.path.exists(lgp_file):
                        size = os.path.getsize(lgp_file)
                        if size != last_size:
                            with progress_data['lock']:
                                progress_data['lgp_size'] = size
                            last_size = size
                except OSError:
                    pass
                time.sleep(0.5)
        ...
        watch_thread = threading.Thread(target=watch_lgp_size)
        ...
        while True:
            ...
            with progress_data['lock']:
                lgp_size = progress_data['lgp_size']
            
            # Mettre à jour seulement si la taille a changé
            if lgp_size != last_lgp_size:
                last_lgp_size = lgp_size
                lgp_size_str = format_size(lgp_size)
                self.progress.emit(0, f"{total_size_str} → {lgp_size_str}")
    ...
    def restore_files(self):
        """Restaure les fichiers originaux depuis .save et .extra, avec restauration des symlinks"""
        print(f"DEBUG: restore_files called")
        print(f"DEBUG: game_dir = {self.game_dir}")
        
        # Charger les infos des symlinks sauvegardés
        symlinks_backup = {}
        symlinks_file = os.path.join(self.game_dir, '.symlinks_backup')
        if os.path.exists(symlinks_file):
            ...
            print(f"DEBUG: Loaded {len(symlinks_backup)} symlinks from backup")
        ...
        if os.path.exists(savepath_file) and os.path.exists(saves_backup_dir):
            ...
            with open(savepath_file, 'r') as f:
                for line in f:
                    ...
                    # Si c'était un symlink original, le restaurer
                    if rel_path in symlinks_backup:
                        ...
                        print(f"DEBUG: Restored symlink {original_path} -> {original_target}")
                    else:
                        # Sinon, restaurer depuis backup (contenu uniquement pour les dossiers)
                        os.makedirs(os.path.dirname(original_path), exist_ok=True)
...
class LGPWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Titre
        self.title_label = QLabel("Création du paquet LGP")
        title_font = QFont()
        ...
        self.args_input.setPlaceholderText("ex: --fullscreen --debug")
    ...
    def load_game_directory(self, game_dir):
        ...
        # Rechercher les exécutables Linux (binaires, AppImages, .sh, .py)
        self.exe_files = []
        for root, dirs, files in os.walk(self.game_dir):
            ...
            for file in files:
                ...
                rel_path = os.path.relpath(full_path, self.game_dir)
                
                # Vérifier si c'est un exécutable, AppImage, script .sh ou .py
                file_lower = file.lower()
                if file_lower.endswith('.appimage') or file_lower.endswith('.AppImage'):
                    ...
                    # Vérifier si c'est un binaire ELF (pas un script texte)
                    if self._is_elf_binary(full_path):
                        self.exe_files.append(rel_path)
                elif os.path.islink(full_path):
                    # Si c'est un symlink, vérifier la cible
                    target = os.readlink(full_path)
                    target_lower = target.lower()
                    if target_lower.endswith('.appimage') or target_lower.endswith('.AppImage'):
                        self.exe_files.append(rel_path)
                    elif target_lower.endswith('.sh'):
                        self.exe_files.append(rel_path)
                    elif target_lower.endswith('.py'):
                        self.exe_files.append(rel_path)
                    elif os.path.exists(target) and os.access(target, os.X_OK):
                        self.exe_files.append(rel_path)
    ...
    def _is_elf_binary(self, filepath):
        """Vérifie si un fichier est un binaire ELF"""
        try:
    ...
    def select_icon_for_exe(self, exe_path):
        ...
        # Priorité 2: Si c'est un AppImage, chercher une icône extraite
        if '.appimage' in exe_lower or '.AppImage' in exe_lower:
            for idx, icon in enumerate(self.available_icons):
                if icon['source'] == 'appimage':
                    self.select_icon(idx)
                    return
        
        # Priorité 3: Chercher une icône correspondant au nom de l'exécutable
        for idx, icon in enumerate(self.available_icons):
            icon_name_lower = icon['name'].lower()
            if exe_name.lower() in icon_name_lower or icon_name_lower in exe_name.lower():
                self.select_icon(idx)
                return
    ...
    def on_exe_selected(self, index):
        ...
        if index >= 0 and index < len(self.exe_files):
            exe_path = self.exe_files[index]
            self.select_icon_for_exe(exe_path)
            
            # Si c'est un AppImage, essayer d'extraire l'icône
            exe_lower = exe_path.lower()
            if '.appimage' in exe_lower or '.AppImage' in exe_lower:
                full_exe_path = os.path.join(self.game_dir, exe_path)
                # Résoudre le symlink si nécessaire
                if os.path.islink(full_exe_path):
                    real_path = os.path.realpath(full_exe_path)
                    if os.path.exists(real_path):
                        self.extract_appimage_icon(real_path)
                else:
                    self.extract_appimage_icon(full_exe_path)
    
    def extract_appimage_icon(self, appimage_path):
        """Extrait l'icône depuis un AppImage"""
        try:
            import tempfile
            
            # Vérifier si l'icône de cet AppImage est déjà dans la liste
            appimage_name = os.path.splitext(os.path.basename(appimage_path))[0]
            for icon in self.available_icons:
                if icon['source'] == 'appimage' and appimage_name[:12] in icon['name']:
                    # Déjà présent, juste la sélectionner
                    idx = self.available_icons.index(icon)
                    self.select_icon(idx)
                    return
            
            # Utiliser appimageiconextract.py pour extraire l'icône
            temp_png = os.path.join(tempfile.gettempdir(), f'lgp_icon_{uuid.uuid4().hex}.png')
            # Trouver appimageiconextract.py dans le même répertoire que ce script
            script_dir = os.path.dirname(os.path.abspath(__file__))
            extractor_path = os.path.join(script_dir, 'appimageiconextract.py')
            # Fallback vers le chemin système si le script n'est pas trouvé localement
            if not os.path.exists(extractor_path):
                extractor_path = '/usr/share/ublue-os/gablue/scripts/appimageiconextract.py'
            result = subprocess.run(
                ['/usr/bin/python3', extractor_path, appimage_path, temp_png],
                capture_output=True,
                timeout=30
            )
    ...
    def load_available_icons(self):
        ...
        for root, dirs, files in os.walk(self.game_dir):
            ...
            for file in files:
                file_lower = file.lower()
                if file_lower.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.svg')):
                    full_path = os.path.join(root, file)
    ...
    def select_custom_icon(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "Images (*.png *.ico *.jpg *.jpeg *.bmp *.svg)"
        )
        if file_path:
            ...
            if file_path:
                ...
                if not file_path.startswith(self.game_dir + os.sep) and file_path != self.game_dir:
                    ...
                    reply = QMessageBox.question(
                        ...
                        f"(Sinon, il ne sera pas inclus dans le paquet LGP)",
    ...
    def update_saves_list(self):
        ...
        for item_type, path in self.saves:
            ...
            self.saves_list.addItem(f"{prefix}{path}")
    
    def update_extras_list(self):
        """Met à jour l'affichage de la liste des extras"""
    ...
    def start_create_lgp(self):
        ...
        if os.path.exists(lgp_file):
            reply = QMessageBox.question(
                ...
                f"{os.path.basename(lgp_file)} existe déjà. Voulez-vous l'écraser ?",
                QMessageBox.Yes | QMessageBox.No
            )
        ...
        self.progress_dialog.setWindowTitle("Compression LGP")
    ...
    def on_finished(self, success, message):
        """Appelé lorsque la création est terminée"""
        # Réinitialiser le flag
        self.is_creating = False
        self.create_btn.setEnabled(True)
        self.create_btn.setText("Créer le LGP")
        ...
        if success:
            ...
            size_after = os.path.getsize(lgp_file)
            size_after_formatted = self.get_file_size(lgp_file)
            ...
            QMessageBox.information(
                self, "Succès",
                f"LGP créé avec succès !\n\n"
                f"Fichier: {os.path.basename(lgp_file)}\n"
...
def main():
    ...
    # Définir le style sombre par défaut
    app.setStyle('Fusion')
