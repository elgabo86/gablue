...
13. **GRUB** : Les noms d'entrées ne doivent pas contenir d'apostrophes (Titanoboa génère `menuentry '...'` sans échapper les apostrophes internes, ce qui casse le parsing GRUB et ne montre qu'une seule entrée)
 ...
 16. **Pré-initialisation du préfixe Wine dans le live** : après le pack cache, `build.sh` pré-initialise un préfixe Wine complet pour la session live. Le préfixe et le runner sont stockés dans `/usr/share/gablue/wine-home/` et `/usr/share/gablue/wine-runner/` (squashfs), avec un symlink `~/Windows → /usr/share/gablue/wine-home` pour que les chemins du registre pointent vers `/home/liveuser/...`. L'init est lancée via `xvfb-run -a env __GLX_VENDOR_LIBRARY_NAME=mesa __EGL_VENDOR_LIBRARY_FILENAMES=/usr/share/glvnd/egl_vendor.d/50_mesa.json LIBGL_ALWAYS_SOFTWARE=1 HOME=/home/liveuser gwine --init --offline` (Xvfb requis pour PhysX/OpenAL, installateurs `.exe` qui nécessitent `CreateWindow` ; les 3 vendor selections force le software rendering Mesa pour éviter un crash Xvfb sur les images NVIDIA où `libnvidia-egl-gbm.so.1` tente d'init un GPU absent). Le cache est extrait temporairement pour l'init puis supprimé (seule l'archive `/extra` persiste pour l'install système). `find ... -exec chown 1000:1000 {} +` (au lieu de `chown -R`, race condition sur les `.tmp` Wine entre la lecture du dossier et le `chown()`) sur les cibles dans `/usr/share/gablue/` pour que liveuser (UID 1000) soit propriétaire au boot. **Livesys** crée l'utilisateur normalement : le home ne contient que des symlinks (quelques octets), pas de copy-up massif. Au live, Wine lit depuis le squashfs (0 RAM), les modifications (registre, saves) déclenchent un copy-up overlay ciblé (quelques fichiers, pas les 2 Go du préfixe)
