...
5. Build de l'image avec buildah (KERNEL_FLAVOR passé via kernel_type, NVIDIA_FLAVOR si fourni) — **retry** via `nick-fields/retry@v4` avec `retry_on_exit_code: "1"` et `timeout_minutes: 60` : le script shell détecte les erreurs réseau (EOF, TLS handshake timeout, connection refused/reset, DNS, Curl timeout, etc.) et sort avec le code 1 (retry), les erreurs de build (échec d'un script RUN) sortent avec le code 2 (échec immédiat, pas de retry). Le `timeout_minutes` de l'action est le seul mécanisme de timeout fiable dans les workflows réutilisables (le `timeout-minutes` natif GitHub Actions est ignoré dans ce contexte). Nettoyage `buildah rmi raw-img` au début de chaque tentative réseau (les blobs déjà téléchargés restent en cache et ne sont pas retéléchargés)
