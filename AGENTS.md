...
**Version du kernel** :
- **Par défaut** : suit automatiquement le kernel **réellement publié sur l'image stable Bazzite** — lu depuis le label OCI `ostree.linux` de `ghcr.io/ublue-os/bazzite:stable` via `skopeo inspect` (retry 3×10s), suffixe `.x86_64` retiré pour former le tag akmods. **Ne PAS parser le `build.yml` de la branche `main`** : il pinne souvent un kernel RC en cours de préparation (ex. 7.2.0-ogc3.rc7.1 alors que le stable publié est 7.1.5) — seul le label de l'image publiée reflète le vrai stable. Plus besoin de commit pour changer de kernel
- **Vérification d'existence** : le tag akmods correspondant est validé via `skopeo inspect` (akmods commun + `akmods-{NVIDIA_FLAVOR}` si variante NVIDIA), sinon fallback
