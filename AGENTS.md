...
13. **GRUB** : Les noms d'entrées ne doivent pas contenir d'apostrophes (Titanoboa génère `menuentry '...'` sans échapper les apostrophes internes, ce qui casse le parsing GRUB et ne montre qu'une seule entrée)
 ...
 16. **Pré-initialisation du préfixe Wine dans le live** : après le pack cache, `build.sh` pré-initialise un préfixe Wine complet pour la session live. Le préfixe et le runner sont stockés dans `/usr/share/gablue/wine-home/` et `/usr/share/gablue/wine-runner/` (squashfs), avec un symlink `~/Windows → /usr/share/gablue/wine-home` pour que les chemins du registre pointent vers `/home/liveuser/...`. L'init est lancée via `xvfb-run -a env HOME=/home/liveuser gwine --init --offline` (Xvfb requis pour PhysX/OpenAL, installateurs `.exe` qui nécessitent `CreateWindow`). Le cache est extrait temporairement pour l'init puis supprimé (seule l'archive `/extra` persiste pour l'install système). `chown -R 1000:1000` sur les cibles dans `/usr/share/gablue/` pour que liveuser (UID 1000) soit propriétaire au boot. **Livesys** crée l'utilisateur normalement : le home ne contient que des symlinks (quelques octets), pas de copy-up massif. Au live, Wine lit depuis le squashfs (0 RAM), les modifications (registre, saves) déclenchent un copy-up overlay ciblé (quelques fichiers, pas les 2 Go du préfixe)
 17. **Patch Titanoboa `-all-root`** : `installer/titanoboa_build_iso.sh` est une copie modifiée du `build_iso.sh` de Titanoboa, bind-mountée par `local-build/build-iso.sh`. La seule différence : le `-all-root` est retiré de l'appel `mksquashfs`, ce qui préserve les UID/GID du payload (UID 1000 pour les fichiers Wine). Sans ce patch, tout le squashfs est root:root et liveuser ne peut pas écrire dans le préfixe. **Pour le CI**, le patch n'est pas appliqué → les ISO CI n'ont pas de préfixe Wine pré-initialisé (cache pack uniquement). Le fork/patch de Titanoboa pour le CI est à prévoir si l'on souhaite la feature en production.
