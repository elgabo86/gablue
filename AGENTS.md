...
- **Gaming** : sunshine, mangohud, terra-gamescope + terra-gamescope-libs (.x86_64/.i686, depuis terra-extras comme Bazzite — le COPR bazzite ne build plus gamescope et Fedora n'a pas de multilib i686 ; terra-gamescope provides/conflicts `gamescope`, le binaire `/usr/bin/gamescope` reste identique) ; `8bitdo-udev-rules` (règles udev manettes 8BitDo depuis Terra : accès utilisateur `uaccess` hidraw pour le dongle 2.4GHz et le Bluetooth — aligné Bazzite `805f8b7`)
...
- **Auto-sortie** : fermeture du clavier via son UI détectée (active && !visible persistant 1,5 s) → le pont se termine tout seul. Autres raccourcis de fermeture : **Start**, Home+Carré, Home+R3 (→ souris)
