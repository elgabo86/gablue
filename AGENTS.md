...
- **Gaming** : sunshine, terra-mangohud (.x86_64/.i686, migré du COPR bazzite vers Terra — aligné Bazzite `a390d80` : le COPR bazzite abandonne progressivement ses paquets (gamescope déjà sorti), terra-mangohud est le même paquet (même spec, version et packager KyleGospo), `Provides: mangohud`, chemins `/usr/bin/mangohud` + `/usr/lib64/mangohud` + `/usr/lib/mangohud` (i686) identiques) + terra-gamescope + terra-gamescope-libs (.x86_64/.i686, depuis terra-extras comme Bazzite — le COPR bazzite ne build plus gamescope et Fedora n'a pas de multilib i686 ; terra-gamescope provides/conflicts `gamescope`, le binaire `/usr/bin/gamescope` reste identique) ; `8bitdo-udev-rules` (règles udev manettes 8BitDo depuis Terra : accès utilisateur `uaccess` hidraw pour le dongle 2.4GHz et le Bluetooth — aligné Bazzite `805f8b7`)
